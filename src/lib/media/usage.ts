...
export type MediaUsage = {
  ...
  readonly postGallery: number;
  readonly categoryThumbnail: number;
...
export async function getMediaUsage(mediaId: string): Promise<MediaUsage> {
  ...
  const [extra] =
    await rawSql`select (select count(*)::int from post_media where media_id=${mediaId}) gallery, (select count(*)::int from categories where thumbnail_media_id=${mediaId}) thumbnail`;

  const usage = {
    postGallery: extra?.gallery ?? 0,
    categoryThumbnail: extra?.thumbnail ?? 0,
...
export function describeUsage(usage: MediaUsage): string[] {
  const parts: Array<[number, string]> = [
    ...
    [usage.postGallery, "แกลเลอรีข่าว"],
    [usage.categoryThumbnail, "ภาพย่อหมวดหมู่"],
