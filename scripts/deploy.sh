...
if ! docker run --rm \
  --network "$(docker network ls --filter name=cida-ppt_default --format '{{.Name}}' | head -1)" \
  --env-file "$ENV_FILE" \
  -e DATABASE_URL="postgres://cida:${POSTGRES_PASSWORD}@db:5432/cida" \
  --entrypoint node "$NEW_IMAGE" \
  scripts/migrate.mjs 2>&1; then
