...
if [[ -n "$SSH_PUBKEY" ]]; then
  log "ssh hardening"
  cat > /etc/ssh/sshd_config.d/10-cida-ppt.conf <<'SSHCONF'
PermitRootLogin no
...
cat > /etc/fail2ban/jail.d/cida-ppt.conf <<'JAIL'
[sshd]
enabled = true
maxretry = 5
findtime = 10m
...
echo 'vm.swappiness=10' > /etc/sysctl.d/60-cida-ppt.conf
sysctl -p /etc/sysctl.d/60-cida-ppt.conf >/dev/null
...
cat > /etc/cron.weekly/cida-ppt-docker-prune <<'CRON'
#!/bin/sh
docker system prune -af --filter "until=168h" >/dev/null 2>&1
CRON
chmod +x /etc/cron.weekly/cida-ppt-docker-prune
