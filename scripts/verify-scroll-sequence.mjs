...
const playerPath = join(root, "src/lib/vocational/scroll-player.ts");
const source = await readFile(playerPath, "utf8");
const player = ts.transpileModule(source, {
  compilerOptions: { target: ts.ScriptTarget.ES2022, module: ts.ModuleKind.ES2022 },
}).outputText;
const css = await readFile(join(root, "src/app/vocational.css"), "utf8");
const sequenceCss = await readFile(
  join(root, "src/components/vocational/scroll-sequence.css"),
  "utf8",
);
...
function fixture(scenario) {
  return `<!doctype html><html lang="th"><head><meta charset="utf-8">
    ...
    <style>
      ...
      *,::before,::after {box-sizing:border-box} body,h1,p {margin:0} body {font-family:Anuphan,sans-serif}
    ...
    <section class="cida-scroll-sequence">
      <div class="cida-sequence-stage">
        <canvas class="cida-sequence-canvas" aria-hidden="true"></canvas>
        <div class="v-hero-image"></div><div class="v-hero-shade"></div>
        <div class="v-hero-content">
          <p class="v-kicker"><span></span>ฝ่ายฝึกวิชาชีพผู้ต้องขัง</p>
          <h1>ฝึกอาชีพ<br><span>สร้างโอกาสใหม่</span></h1>
          <p class="v-hero-description">พื้นที่แห่งการเรียนรู้และพัฒนาทักษะวิชาชีพ
            <br class="v-desktop"> ผ่านการลงมือทำจริง สู่ผลงานที่มีคุณค่า
            <br class="v-desktop"> และโอกาสในวันข้างหน้า</p>
          <div class="v-actions">
            <a class="v-button v-button-light" href="#products">ชมผลิตภัณฑ์ <span>↗</span></a>
            <a class="v-text-link" href="#vocational">เรียนรู้เรื่องงานฝึกวิชาชีพ <span>↗</span></a>
          </div>
        </div>
        <div class="v-hero-bottom"><span>CRAFTED WITH PURPOSE.<br>SKILLS FOR A NEW BEGINNING.</span>
          <p>ความตั้งใจ สร้างคุณค่าในทุกชิ้นงาน</p><small>ภาพประกอบแนวคิดงานฝีมือ</small></div>
      </div>
    </section><main class="fixture-after">Following page content</main>
    <script type="module">
      import { startScrollSequence } from '/scroll-player.js';
      await document.fonts.ready;
      window.startPlayer = () => startScrollSequence(document.querySelector('.cida-scroll-sequence'), document.querySelector('canvas'), ${JSON.stringify(scenario)});
...
const server = createServer(async (request, response) => {
  try {
    ...
    } else if (url.pathname === "/scroll-sequence.css") {
      response.writeHead(200, { "Content-Type": "text/css" }).end(sequenceCss);
    } else if (url.pathname === "/scroll-player.js") {
      response.writeHead(200, { "Content-Type": "text/javascript" }).end(player);
...
async function scrollToProgress(page, progress) {
  await page.evaluate((value) => {
    const section = document.querySelector(".cida-scroll-sequence");
    const stage = document.querySelector(".cida-sequence-stage");
...
async function assertContentNotClipped(page) {
  const result = await page.evaluate(() => {
    const stage = document.querySelector(".cida-sequence-stage").getBoundingClientRect();
    const nodes = [
      ...document.querySelectorAll(".v-hero-content h1, .v-actions a, .v-hero-bottom"),
    ];
...
try {
  ...
  for (const progress of [0, 0.25, 0.5, 0.75, 1, 0.75, 0.5, 0.25, 0]) {
    ...
    await expectFrame(normal.page, Math.round(149 * progress) + 1, `scroll ${progress * 100}%`);
  }
  ...
  await expectFrame(normal.page, 150, "rapid jumps converge");
  ...
  await expectFrame(normal.page, 150, "restart after cleanup");
  assert.deepEqual(normal.errors, []);
  await normal.context.close();

  const late = await openFixture(browser, "late");
  await scrollToProgress(late.page, 99 / 149);
  await delay(250);
  ...
  await expectFrame(missing.page, 150, "playback continues after failed frame");
  ...
  for (const viewport of [
    ...
    await expectFrame(mobile.page, 150, `${viewport.width}×${viewport.height} endpoint`);
    await assertContentNotClipped(mobile.page);
  ...
  await expectFrame(reduced.page, 150, "motion preference can enable playback");
