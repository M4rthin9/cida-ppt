...
# --- media ------------------------------------------------------------------
# Media includes every authoritative original and its responsive derivatives.
# Fail the backup if the archive fails; never publish a partial archive as success.
tar="$BACKUP_DIR/daily/media-$stamp.tar.gz"
log "archiving $MEDIA_DIR"
tar -czf "$tar.partial" -C "$MEDIA_DIR" .
mv "$tar.partial" "$tar"
